...
import { BolsoDB } from './BolsoDB.js';
 ...
 * @param {Date}   [opcoes.dataRef]  — Data que será carimbada.
 * @param {Array}  [opcoes.metas]    — Lista de metas vindas do MetaDB.
 * @param {Function} [opcoes.onMetaContribute] — Callback para MetaDB.contribuir(id, valor, 'autopilot').
 * @returns {{...}}
 */
function virar_mes({ mesAlvo, dataRef, metas = [], onMetaContribute } = {}) {
    ...
    // ─────────────────────────────────────────────────────
    //  PASSO 4 — Alocação de Metas (Autopilot)
    // ─────────────────────────────────────────────────────
    let totalMetas = 0;
    const agendamentosAtivos = metas.filter(m => m.agendamento && m.agendamento.ativo);

    if (agendamentosAtivos.length > 0) {
        console.group('🎯 Passo 4 — Alocação de Metas');
        for (const meta of agendamentosAtivos) {
            const valorAporte = meta.agendamento.valor;
            ...
            saldoAtual = Number((saldoAtual - valorAporte).toFixed(2));
            totalMetas = Number((totalMetas + valorAporte).toFixed(2));
            
            BolsoDB._adicionarATransacao(lancamento);
            lancamentos.push(lancamento);

            // Side-effect: atualiza o saldo interno da meta no MetaDB (se callback fornecido)
            if (typeof onMetaContribute === 'function') {
                onMetaContribute(meta.id, valorAporte, 'autopilot');
            }

            console.log(`  ✅ -${_fmt(valorAporte)} → Meta: "${meta.nome}"`);
        }
        console.log(`  📊 Total aportado em metas: -${_fmt(totalMetas)} → Saldo: ${_fmt(saldoAtual)}`);
        console.groupEnd();
    }
    ...
    console.log(`  - Metas      : -${_fmt(totalMetas)}`);
    console.log(`  ─────────────────────────`);
