...
function criarAppStateVazio() {
    return {
        ...
        ganhosMensais: [],
        gastosMensais: [],
        // ── Configurações do sistema (Issue #21) ──────────────
        configuracoes: {
            diaVencimentoCartao:   null, // number 1–28 | null
            diaRecebimentoSalario: null, // number 1–28 | null
            limiteCartao:          0,    // R$ — base para alertas de risco
            ultimoMesProcessado:   null, // "YYYY-MM" — lacre de idempotência
            plano: 'gratuito',           // 'gratuito' | 'pago'
        },
    };
}
...
function carregarEstado() {
    try {
        ...
        if (raw) {
            ...
            // Mescla superficial: preserva chaves novas (ex: configuracoes) para usuários
            // que tinham dados antes desta versão do schema.
            const base = criarAppStateVazio();
            _state = { ...base, ...parsed };
            // Migração profunda de configuracoes: garante subcampos novos
            _state.configuracoes = { ...base.configuracoes, ...(parsed.configuracoes ?? {}) };
...
/**
 * Retorna uma cópia das configurações do sistema.
 * @returns {object}
 */
...
/**
 * Atualiza parcialmente as configurações (patch merge).
 * Só os campos presentes em `patch` serão alterados.
 * @param {object} patch — ex: { diaVencimentoCartao: 10, limiteCartao: 2000 }
 */
function salvarConfiguracoes(patch) {
    if (!patch || typeof patch !== 'object') return;
    _state.configuracoes = { ..._state.configuracoes, ...patch };
    salvarEstado();
    console.info('[DB] ✅ Configurações salvas:', _state.configuracoes);
}
 ...
 */
export const BolsoDB = { //Este objeto está sendo declarado dentro de um contexto local, por conta da declaração com "const", logo, ele não será atributo do objeto global "window".
    // ── Setup ──────────────────────────────────────────
    init: initDB,
    reset: resetarEstado,
    getEstado,
